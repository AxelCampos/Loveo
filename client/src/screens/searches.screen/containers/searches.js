    ...
    deleteSearch: id => mutate({
      ...
      /* ,
            update: (store, { data: { deleteSearch } }) => {
                ...
                const data = store.readQuery({ query: SEARCH_QUERY, variables: { id: 1 } }); // TODO: si se descomenta este troncho de código poner la id de auth
                //  Add our message from the mutation to the end.
                data.search = data.search.filter(item => deleteSearch.id !== item.id);
                //  Write our data back to the cache.
                store.writeQuery({
                    query: SEARCH_QUERY,
                    variables: { id: 1 }, // TODO: si se descomenta este troncho de código poner la id de auth
                    data,
                });
            }, */
    }),
